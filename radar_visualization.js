  ...
  function renderRadarList(config) {
      const container = document.getElementById(config.svg + "-list");
      if (!container) return;
      ...
      config.entries.forEach(entry => {
          ...
          item.style.borderLeftColor = config.rings[entry.ring].color;
          ...
          meta.className = "meta";
          meta.textContent = `${config.quadrants[entry.quadrant].name} – ${config.rings[entry.ring].name}`;

          if (entry.link) {
              const link = document.createElement("a");
              link.href = entry.link;
              link.target = config.links_in_new_tabs ? "_blank" : "_self";
